...
KKQ_pilot_data %>%
  ...
  geom_smooth(color = "black", method = "glm", 
              method.args = list(family = "binomial")) +
  geom_jitter(aes(), width = 0, height = 0.2, alpha = 0.65, size=3) +
  scale_x_continuous(limits = c(5.0, 7.0)) +
  ggtitle("Continuous") +
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(breaks = c(0, 1)) +
...
KKQ_pilot_data %>%
  ...
  geom_smooth(method = "glm", se = FALSE,
              method.args = list(family = "binomial"), aes(color = "test_question")) +
  geom_jitter(width = 0, height = 0.2, alpha = 0.65, size=3) +
  scale_x_continuous(limits = c(5.0, 7.0)) +
  facet_wrap(version~ test_question, ncol = 1, scales = "free") +
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  scale_y_continuous(breaks = c(0, 1)) +
  scale_color_manual(values = c("darkred", "navyblue", "black", "grey")) +
  labs(x = "Age", y = "",  color = "Correct") +
  theme(text = element_text(color = "black", size = 24), axis.text = element_text(color = "black", size = 24), 
